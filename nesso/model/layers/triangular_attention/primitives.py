...
from nesso.model.layers.triangular_attention.utils import (
    flatten_final_dims,
    permute_final_dims,
)


_CUEQUIVARIANCE_AVAILABLE = importlib.util.find_spec("cuequivariance_torch") is not None
...
def kernel_triangular_attn(q, k, v, tri_bias, mask, scale):
    ...
    return triangle_attention(q, k, v, tri_bias.float(), mask=mask, scale=scale)
...
class Attention(nn.Module):
    ...
    def forward(
        ...
        """
        use_kernel_path = use_kernels and _CUEQUIVARIANCE_AVAILABLE
        # Attention kernel applies scaling internally
        q, k, v = self._prep_qkv(
            q_x,
            kv_x,
            apply_scale=not use_kernel_path,
        )
